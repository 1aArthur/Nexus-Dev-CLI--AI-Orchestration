...
void main() {
  testWidgets(
    'package cards expose provenance signature license and permissions',
    (tester) async {
      ...
      await tester.scrollUntilVisible(find.text('Signature · Verified'), 200);

      expect(find.text('Signature · Verified'), findsOneWidget);
      expect(find.text('License · Apache-2.0'), findsOneWidget);
      expect(
        find.text('Permissions · catalog:read, templates:import'),
        findsOneWidget,
      );
      expect(
        find.textContaining('github.com/nexu-io/open-design'),
        findsOneWidget,
      );
  ...
  testWidgets('unlicensed provider pets remain visible but disabled', (
    ...
    expect(
      find.text('Unavailable · authorized redistributable source required'),
      findsOneWidget,
    );
