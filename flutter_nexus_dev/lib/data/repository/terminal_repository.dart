...
class TerminalRepository {
  ...
  List<TerminalCommandEntity> _getAllCommands() => _box.values.toList().reversed.toList();
  ...
  (String tag, String output) parseAndGenerateOutput(String input, TerminalShellType shellType) {
    ...
    switch (lower) {
      case 'help': return ('SHELL', _getHelpText(shellType));
      case 'clear': return ('SHELL', 'CLEAR_TERMINAL');
      case 'ls': case 'dir': return ('SHELL', _getLsOutput());
      case 'pwd': return ('SHELL', '/data/user/0/com.nexus.dev.orchestrator');
      case 'whoami': return ('SHELL', 'nexus-dev@android');
      case 'date': return ('SHELL', DateTime.now().toIso8601String());
      case 'ps': return ('SHELL', _getPsOutput());
      case 'top': return ('SHELL', _getTopOutput());
      case 'df': return ('SHELL', _getDfOutput());
      case 'free': return ('SHELL', _getFreeOutput());
      case 'env': return ('SHELL', _getEnvOutput());
      case 'history': return ('SHELL', _getHistoryOutput());
    }
    if (lower.startsWith('nx ')) return _parseNxCommand(lower.substring(3));
...
SHELL-SPECIFIC:
  ...
  fish          Switch to Fish
''';
  String _getLsOutput() => '''drwxr-xr-x  2 nexus  nexus  4096 Aug 19 12:00 app
...
-rw-r--r--  1 nexus  nexus  1024 Aug 19 12:00 LICENSE''';
  String _getPsOutput() => '''  PID  PPID  NAME
    1     0    init
  123     1    zygote
